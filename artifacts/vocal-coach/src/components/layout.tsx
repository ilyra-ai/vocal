...
import { Mic2, LayoutDashboard, History, User, MessageSquare, Music, Menu, X } from "lucide-react";
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  const SidebarContent = () => (
    <>
      <div className="h-16 flex items-center px-6 border-b border-sidebar-border shrink-0">
        ...
        <button
          className="ml-auto md:hidden text-muted-foreground hover:text-white"
      ...
      <div className="p-4 border-t border-sidebar-border shrink-0">
        <div className="flex items-center p-3 rounded-xl bg-sidebar-accent/50">
          <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center border border-primary/30 mr-3 overflow-hidden shrink-0">
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <aside className="hidden md:flex w-64 border-r border-sidebar-border bg-sidebar flex-col shrink-0 relative z-20">
        <SidebarContent />
      </aside>
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-40 w-72 bg-sidebar border-r border-sidebar-border flex flex-col md:hidden transition-transform duration-300 ease-in-out",
      ...
      <main className="flex-1 flex flex-col relative z-10 overflow-hidden min-w-0">
        ...
        {/* Top bar mobile */}
        <div className="md:hidden h-14 flex items-center px-4 border-b border-sidebar-border bg-sidebar/80 backdrop-blur-md shrink-0 z-10">
          <button
